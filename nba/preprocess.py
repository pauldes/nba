...
def scale_per_value_of(data, selected_cat_features, selected_num_features, fit_per_value_of, min_max_scaler=True):
    ...
    processed_num_data = standardize(data[selected_num_features], fit_on=None, fit_per_values_of=fit_per_value_of, min_max_scaler=min_max_scaler)
    if selected_cat_features is not None and len(selected_cat_features)>0:
        processed_cat_data = pandas.get_dummies(data[selected_cat_features]) #, drop_first=True)
